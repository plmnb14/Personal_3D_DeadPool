...
CKeyMgr::CKeyMgr()
	: m_dwKey(0), m_dwKeyDown(0), m_dwKeyUp(0), m_fDPI(0.5f),
...
void CKeyMgr::Set_Handle(HWND _g_hWnd)
{
	m_hWnd = _g_hWnd;
}

HWND CKeyMgr::Get_Handle()
{
	return m_hWnd;
}

void CKeyMgr::Set_DPI(_float _DPI)
{
	m_fDPI = _DPI;
}

void CKeyMgr::Set_MouseLock()
{
	POINT pt;
	pt.x = WINCX / 2;
	pt.y = WINCY / 2;
...
POINT CKeyMgr::Get_ScreenCenter()
{
	POINT pt;
	pt.x = WINCX / 2;
	pt.y = WINCY / 2;
